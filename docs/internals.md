...
with Lanthanide(2) as ion:
    order = 2
    for initial_index, final_index, key_slice in ion.single.elements(order):
        keys = ion.single.lower_keys(key_slice, order)
        value = 0.0
        for key, parity in keys:
            initial, final = ion.single.index_pair(key, order)
