...
The following code provides an example of how to use `SingleElements` with the elementary tensor operator evaluation
function `operator(initial, final)` of your own `num`-electron tensor operator. Note that the actual quantum numbers
