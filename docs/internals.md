...
generators can be converted into tuples of final and initial electrons by the method `index_pair(key, num)`.

The following code provides an example of how to use the elementary tensor operator evaluation function
`operator(initial, final)` of your own `num`-electron tensor operator. Note that the actual quantum numbers $m_l$ and
$m_s$ of the initial and final electrons are only required for the final evaluation of the `operator` function. They
are extracted from the constant tuple `MAGNETIC` of all f electrons in standard order. We calculate only elements
in the lower triangle of the matrix. This would be suitable for a symmetric operator.
